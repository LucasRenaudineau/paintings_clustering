...
# the 10 layers (5 full image + 5 uniform crop) are concatenated into a single
# 7808-dim float32 vector. Full-image block (indices 0..3903):
...
# Uniform-crop block (indices 3904..7807): same layers/widths, scaled by
# CROP_WEIGHT, offset by 3904.
...
# Half-open [start, end) channel ranges for all 10 activations in the 7808-dim
# GAP vector, with the matching per-layer weight (must mirror LAYER_WEIGHTS in
# features_comparator). Indices 0-4 are the full-image block; 5-9 the crop block.
_LAYER_SPANS = (
    (0,    64,   1.0),   # 0  conv1_relu        full image
    (64,   320,  1.0),   # 1  conv2_block3_out   full image
    (320,  832,  0.1),   # 2  conv3_block4_out   full image
    (832,  1856, 0.1),   # 3  conv4_block6_out   full image
    (1856, 3904, 0.1),   # 4  conv5_block3_out   full image
    (3904, 3968, 1.0),   # 5  conv1_relu         uniform crop
    (3968, 4224, 1.0),   # 6  conv2_block3_out   uniform crop
    (4224, 4736, 1.0),   # 7  conv3_block4_out   uniform crop
    (4736, 5760, 0.1),   # 8  conv4_block6_out   uniform crop
    (5760, 7808, 0.1),   # 9  conv5_block3_out   uniform crop
)
